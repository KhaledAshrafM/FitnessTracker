...
df[outlier_columns[:3] + ["label"]].boxplot(by="label", layout=(1, 3))
df[outlier_columns[3:] + ["label"]].boxplot(by="label", layout=(1, 3))
plt.show()
...
def mark_outliers_iqr(dataset: pd.DataFrame, col: str) -> pd.DataFrame:
    ...
    Args:
        dataset (pd.DataFrame): the dataset
        col (string): The column we want to apply outlier detection to

    Returns:
        pd.DataFrame: The original dataframe with an extra boolean column
        indicating whether the value is an outlier
    """
...
# ---------------------------------------------------------
# Chauvenet's criterion (a normal distribution is assumed)
# ---------------------------------------------------------
...
def mark_outliers_chauvent(dataset: pd.DataFrame, col: str, C=2) -> pd.DataFrame:
    """Finds outliers in the specified columns of datasets and adds a binary column with
    the same name extended with '_outlier' that expresses the results per data point.
    ...
    Args:
        dataset (pd.DataFrame): The dataset
        col (string): The column we want to apply outlier detection to
        C (int, optional): Degree of certainty for the identification of outliers given the
        assumption of a normal distribution, defaults to 2.
    ...
    # Pass all rows in the datasets
    for i in range(N):
        # Determine the probability of observing the point
        prob.append(
            1.0 - 0.5 * (scipy.special.erf(high[i]) - scipy.special.erf(low[i]))
        )
        # And mark as an outlier when the probability is below the criterion.
        mask.append(prob[i] < criterion)
    dataset[col + "_outlier"] = mask
    return dataset

# Loop over all columns (again)
for col in outlier_columns:
    dataset = mark_outliers_chauvent(df, col)
    plot_binary_outliers(dataset, col, col + "_outlier", True)
...
def mark_outliers_lof(dataset, columns, n=20):
    ...
    Args:
        dataset (pd.DataFrame): The Dataset
        columns (string): The columns you want apply outlier detection to
        n (int, optional): n_neighbors. Defaults to 20.

    Returns:
        pd.DataFrame: The original dataframe with an extra boolean column
        indicating whether the value is outlier or not.
    """
...
for col in outlier_columns:
    dataset = mark_outliers_chauvent(df[df["label"] == label], col)
    plot_binary_outliers(dataset, col, col + "_outlier", reset_index=True)
...
outliers_removed_df.to_pickle("../../data/intermediate/02_outliers_removed_chauvent.pkl")
